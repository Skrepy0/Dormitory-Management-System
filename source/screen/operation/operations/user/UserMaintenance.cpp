...
void UserMaintenance::submitRepair()
{
    ...
    if (isConfirm) showSuccess("screen.operation.operations.UserMaintenance.repair.success");
    else showError("screen.operation.operations.UserMaintenance.repair.error");

    std::string currentTimeStr = getCurrentTime();
    int year, month, day, hour, minute, second;

    if (sscanf(currentTimeStr.c_str(), "%d-%d-%d %d:%d:%d",

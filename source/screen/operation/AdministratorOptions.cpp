...
// 获取管理员账号（专属校验：6位纯数字）
std::string AdminOptions::getAdminIdInput()
{
    return getDigitInput("请输入管理员账号：", 6, 6);
}
...
std::string AdminOptions::getDormNumInput()
{
    ...
    while (true)
    {
        ...
        if (regex_match(dormNum, pattern)) return dormNum;
...
std::string AdminOptions::getAuditOpinionInput()
{
    ...
    while (true)
    {
        ...
        if (opinion.length() <= 30) return opinion;

...
public:    //输出类：
          void showTitle(const string& title);//显示菜单标题
          void clearScreen();//用于清空屏幕
          void showSuccess(const string& message);//显示成功提示
          void showError(const string& message);//显示错误提示
          void showPrompt(const string& message);//显示普通提示文本
          void pause();//暂停等待（按任意键继续）
          //输入类：
          string getDigitInput(const string& prompt,int minlen = 0,int maxlen = 0);//获取纯数字输入（如学号）
          string getNonEmptyInput(const string& prompt);//获取非空字符串输入（如姓名，学号）
          string getPasswordInput(const string& prompt);//获取密码输入
          //确认操作
          bool confirmOperation(const string& prompt);//确认操作

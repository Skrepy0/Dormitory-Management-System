...
private:
	...
	static bool writeLanguageToFile(std::string lang);
	nlohmann::json readFromJson();
public:
	Text();
	Text(std::string path);
	static Text of(std::string content);// 不通过json直接显示content内容 用法：Text::of("刘兴康大帅比");可以获得content为"刘兴康大帅比"的Text对象，不通过path转化
...
/*  使用参考：
	...
	Text::setLanguage("zh_cn");// 设置语言是中文
	std::cout << Text::of("刘兴康大帅比").getContent() << std::endl;// 虽然这样写可以不乱码，尽量减少这样写，因为这样写的文本不受语言的影响，中英文都是"刘兴康大帅比"

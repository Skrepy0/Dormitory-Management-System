...
private:
    ...
    static void buildIndexMaps(); // 通过json文件构建索引map
public:
    Accommodations();

    void addBuildings(BuildingData data); // 添加宿舍楼
    static long long findBuildingByName(std::string name); // 通过宿舍楼的名字来查找宿舍楼，并返回索引
    ...
    bool static writeInFile(nlohmann::json data); // 将data信息写入DormitoryData.json(密码以哈西值的方式存储)
